...
## GitHub repository
https://github.com/danilo90lan/Currency_exchange.git
...
Here’s a brief step-by-step description of how tasks are planned and tracked in your Currency Exchange project using Trello:

[Link to Trello Board](https://trello.com/b/L5PlHgQp/currency-exchange)

1. **Set Up Virtual Environment:** Create and activate a dedicated space for the project to manage dependencies.
2. **Install Dependencies:** Install required Python packages to enable project functionality. (Flask, Marshmallow, SQLAlchemy, Flask-JWT-Extended, Psycopg2, Requests, Bcrypt, and APScheduler)

![Trello tracking](./docs/trello/1.png)
![Trello tracking](./docs/trello/2.png)

3. **Install PostgreSQL**: Set up a PostgreSQL database for data storage.
4. **Initialize the Database**: Create the database and configure user privileges. 
GRANT ALL PRIVILEGES ON DATABASE (databse_name) TO (user_name)
GRANT ALL ON SCHEMA public TO (user_name)

![Trello tracking](./docs/trello/3.png)

5. **Set Up Environment Variables**: Configure essential environment variables for the application.
Create a .env file or set environment variables directly.
Add the following variables:
- DATABASE_URL: Connection string for the database (postgres://username:password@host:port/database_name)
- JWT_SECRET_KEY: A secret key used to sign JSON Web Tokens (JWTs). This key is important for ensuring the integrity and authenticity of the tokens
- FLASK_APP: The name of the application module or package. This variable tells Flask which application to run when executing Flask commands.
- FLASK_DEBUG: A flag that enables or disables debug mode in Flask
- FLASK_RUN_PORT: The port on which the Flask application will run. By default Flask uses port 5000

![Trello tracking](./docs/trello/4.png)
![Trello tracking](./docs/trello/5.png)

6. **Create Models:**  Define the data structure for the application:
- Design models for Account, Currency, Deposit, Exchange, and User.
- Implement each model as a Python class using SQLAlchemy.
- Create corresponding Marshmallow schemas for serialization and validation.

![Trello tracking](./docs/trello/6.png)

7. **CLI Commands:**  Provide command-line utilities for database management.
- Create CLI commands for dropping, creating, and seeding tables.
- Implement these commands using Flask's @app.cli.command decorator.

![Trello tracking](./docs/trello/7.png)

8. **Currency seeding (UTIL folder)** Implement function to retreive and seed currency table
![Trello tracking](./docs/trello/8.png)
![Trello tracking](./docs/trello/9.png)

9. **Auth Controller:** Handle authentication-related functionalities.
- Implement endpoints for user registration, login, updating, and deleting users.
- Add functionality to retrieve all users for admin purposes.
![Trello tracking](./docs/trello/10.png)

10. **Account Controller:** Manage user accounts and their operations.
- Implement endpoints for fetching all accounts grouped by currency_code (admin), adding new accounts, updating account details, and deleting accounts.

![Trello tracking](./docs/trello/11.png)
![Trello tracking](./docs/trello/12.png)

11. **Deposit Controller:** Manage deposits for user accounts.
- Implement endpoints to retrieve deposit history for a specific account.
- Create functionality for new deposit transactions.

![Trello tracking](./docs/trello/13.png)

12. **Exchange Controller:** Facilitate currency exchanges between accounts.
- Implement endpoints for retrieving exchange history.
- Create functionality for executing new exchanges between user accounts.

![Trello tracking](./docs/trello/14.png)
![Trello tracking](./docs/trello/15.png)

13. **Implement a decorator and a function for authorization purpose**
- The decorator is located is in the authorization.py file located in the util folder and it checks if a specified account belongs to the authenticated user.
- The fucntion authorize_as_admin() checks if the specified user has admin privileges by checking if the attribute is_admin is True.

![Trello tracking](./docs/trello/16.png)

14. **Currency Controller:**  Manage currency data from external APIs.
- Implement functionality to fetch and display currency rates from a third-party API (Open Exchange Rates).
-Implement background tasks for seeding and updating currency data using APScheduler which run a scheduler in background, set every 60 min calling the update_exchange_rates() function located in the util folder (currency.py file) 

![Trello tracking](./docs/trello/17.png)
![Trello tracking](./docs/trello/18.png)
![Trello tracking](./docs/trello/19.png)

15. **Input Validation Models:** Ensure that user input meets specific criteria for security and integrity.
- Define validation rules for each attribute in your models.
- Implement these rules using Marshmallow's fields and validations to enforce constraints.

![Trello tracking](./docs/trello/20.png)

16. **Error Handling**: The application handles all types of errors gracefully, providing meaningful feedback to the user while maintaining application stability.

![Trello tracking](./docs/trello/21.png)
...
## PostgreSQL Advantages and Drawbacks
...
- **Atomicity** Atomicity is a property that ensures that a database follows the all or nothing rule, meaning it consider the transaction operations as one single unit. 
When a database processes a transaction, it is either fully completed or not executed at all.
- **Consistency** Consistency is a property ensuring that only valid data following all rules and constraints is written in the database. When a transaction results in invalid data, the database reverts to its previous state.  
- **Isolation** Isolation is a property that guarantees the individuality of each transaction, and prevents them from being affected from other transactions. It ensures that transactions are securely and independently processed at the same time.  
- **Durability** Durability is a property that enforces completed transactions, guaranteeing that once each one of them has been committed, it will remain in the system even in case of subsequent failures. If a transaction is successful, all changes generated by it are stored permanently.

Other Features of PostgreSQL are:

* **Data Integrity**: One of the main features offered by relational databases is data integrity, which ensures that the data contained in the database is reliable and consistent. In PostgreSQL, data integrity is enforced through data types, triggers, and constraints.  
Data types limit the type of data that can be stored in a column.  
Triggers are special database functions that are automatically executed by the DBMS when certain events occur, such as INSERT, UPDATE, or DELETE operations on a table.  
Constraints are rules that ensure that data in tables is accurate and consistent. Specifically, constraints limit what can be added or changed in a table, preventing invalid data from being stored.

* **Handling of Complex Queries**: It handles relational data structures like Foreign Keys, Joins, and Complex Queries. For example in the Currency-exchanging app, PostgreSQL allows performing multi-table joins to retrieve a user's complete financial history across multiple accounts, operations, and even foreign exchanges.


* **JSONB support for flexible data structures**: PostgreSQL it does support JSONB for semi-structured data storage or flexible data storage. JSONB (JSON Binary) is a data type in PostgreSQL that allows you to store and manipulate JSON data in a more effective and efficient way than the regular JSON data type. JSONB stores JSON data in a binary format, which enables faster indexing and query performance compared to the regular JSON data type.

* **Customizability and Extensibility**: Thanks to PostgreSQL's design, programmers can add new custom data types, operators, and functions to the database to expand its capability. It is also helpful for managing particular data types or developing domain-specific features because of its extensibility.

* **Scalability and Performance**: PostgreSQL is built to manage demanding workloads and large-scale applications. Sharding facilitates horizontal scalability and offers performance-enhancing tools like query planning, caching, and parallel query execution. 

* **Advanced Security Features**: PostgreSQL is highly secure because of its advanced security features. It offers robust security features, such as SSL encryption, username/password authentication, LDAP authentication, Kerberos authentication, role-based access control (RBAC), and row-level security (RLS). 
For a currency exchange app dealing with sensitive financial information, protecting user data is very important. 

* **Role-based access control** In a PostgreSQL server, a role is a collection of permissions that it can be assigned to one or more users.

* **Free and Extensible**: PostgreSQL is an open-source database that provides affordable solutions without a license charge.

* **Strong Community Support**: The development community guarantees ongoing development, issue fixes, and timely updates. It also offers a variety of documentation, extensions developed by the community, and resources.
...
## SQLAlchemy (ORM) Features and Functionalities
...
One of the main features of an ORM is its ability to map relational database tables to Python classes. Each row in a table is represented as an instance of the mapped class, while columns are represented as class attributes.  
*Example*
...
new_account = Account(account_name = "SAVINGS", currency_code="EUR", balance=2000)
...
#### Fetch an account
```python
stm = db.select(Account).filter_by(id=1)
account = db.session.scalar(stm)
# update name
account.account_name="Euro-trip"
db.session.commit()
```
#### Deleting records
...
![ERM diagram](./docs/Currency_exchange.drawio.png)
...
A user can have multiple accounts, each storing individual balances and unique currency. Each account is associated with a specific user, facilitating user operations on deposits and exchanges. This is represented as a one-to-many relationship between User and Account. 
...
Accounts can have many deposits which track the funds deposited by the user, but each deposit is linked to a single account. This relationship allows for comprehensive transaction histories tied to individual accounts.
...
Multiple accounts can share the same currency type, but each account is limited to a specific currency. This ensures consistency in transactions within the same currency and prevents duplication of currency types for a user.
...
This endpoint:
- retrieves and counts all user accounts in a system, grouping the results by the currency_code associated with each account.
- Inside the function, the authorize_as_admin() function checks whether the requesting user has administrative privileges. Only admins are allowed to access the data provided by this endpoint.
...
The query returns a list of tuples, where each tuple contains:
The currency_code.
The count of accounts with that currency. This result is then formatted into a list of dictionaries.
...
Currency_code CANNOT be changed.
...
![update account success](./docs/update_account_success.png)  
...
* HTTP Verb: GET
* Route Path: accounts/<int:account_id>/deposit-history
* Authorization: Requires a valid JWT token in the Authorization header.
...
Successful Response (200 OK): Example of 200 code request in the scenario where there are no deposits records for the specified account.
...
* HTTP Verb: POST
* Route Path: accounts/<int:account_id>/deposit
* Authorization: Requires a valid JWT token in the Authorization header.
* The @check_account_user decorator ensures that the user is authorized to update the specified account by verifying the account_id belongs to the authenticated user.
...
* HTTP Verb: GET
* Route Path: accounts/<int:account_id>/exchange-history
* Authorization: Requires a valid JWT token in the Authorization header.
* The @check_account_user decorator ensures that the user is authorized to update the specified account by verifying the account_id belongs to the authenticated user.
...
Successful Response (200 OK): Example of 200 code request in the scenario where there are no exchanges records for the specified account.
...
* Authorization: Requires a valid JWT token in the Authorization header. Use @jwt_required() to ensure only authenticated users can access this data.
...
Across the whole project the comment style used is the **PEP 8** guidelines for writing comments in Python. This includes using block comments, inline comments, and docstrings (Use triple quotes to describe functions and methods, including parameters, return values, and exceptions.)

## Database Initialization
To initialize the database tables, you can run the following CLI commands:

1. Create Database Tables:
```
flask db create
```
This command will create all the necessary tables and seed the currency table for initialization.

2. Populate Database Tables:
```
flask db seed
```
This command populates all the tables with initial data.  
By default all the users that populate the users table are seeded with a default password "ABCDabcd1"

3. Drop All Tables (Optional):
```
flask db drop
```
This command drops and deletes all tables from the database.